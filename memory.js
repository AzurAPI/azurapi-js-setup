...
function parseMemory(doc, name, icon, url) {
  return fuse(
    {
      names: parseNames(
        doc.querySelector("div[title='Chinese Story']"),
        doc.querySelector("div[title='Japanese Story']"),
        doc.querySelector("div[title='English Story']")
      ),
      thumbnail: icon,
      wikiUrl: url,
...
function fuse(obj, name) {
  let { cn, jp, en } = obj;
  let chapters = [];
  for (let i = 0; i < Math.max(cn.length, jp.length, en.length); i++) {
    // Looping through chapters
    let length = Math.max(
      cn[i] ? cn[i].lines.length : 0,
      jp[i] ? jp[i].lines.length : 0,
      en[i] ? en[i].lines.length : 0
    );
    let names = {};
    let lines = [];
    ...
    for (let j = 0; j < length; j++) {
      lines.push({
        ...
        bannerSrc:
          (cn[i] && cn[i].lines[j] ? cn[i].lines[j].bannerSrc : null) ||
          (jp[i] && jp[i].lines[j] ? jp[i].lines[j].bannerSrc : null) ||
          (en[i] && en[i].lines[j] ? en[i].lines[j].bannerSrc : null),
        bgm:
          (cn[i] && cn[i].lines[j] ? cn[i].lines[j].bgm : null) ||
          (jp[i] && jp[i].lines[j] ? jp[i].lines[j].bgm : null) ||
          (en[i] && en[i].lines[j] ? en[i].lines[j].bgm : null),
        background:
          (cn[i] && cn[i].lines[j] ? cn[i].lines[j].background : null) ||
          (jp[i] && jp[i].lines[j] ? jp[i].lines[j].background : null) ||
          (en[i] && en[i].lines[j] ? en[i].lines[j].background : null),
        content: {
...
function parseChapter(div) {
  ...
  for (let i = 2; i < rows.length; i++) {
    ...
    line.bannerSrc =
      ...
      row.firstElementChild.firstElementChild.tagName === "IMG"
        ? galleryThumbnailUrlToActualUrl(row.firstElementChild.firstElementChild.src)
        : null);
    ...
    if (row.lastElementChild && row.lastElementChild.childNodes.length > 0) {
      // Notes
      if (row.lastElementChild.querySelector("img"))
        line.background =
          "https://azurlane.koumakan.jp" +
          galleryThumbnailUrlToActualUrl(row.lastElementChild.querySelector("img").src);
      else line.bgm = row.lastElementChild.textContent;
      if (row.lastElementChild.childNodes.length > 1)
        line.bgm = row.lastElementChild.childNodes[1].textContent.replace("BGM:", "").trim();
    }
    lines.push(line);
  }
  return {
...
function parseNames(cn, jp, en) {
  return {
    cn:
      cn && cn.querySelector(".mw-headline") ? cn.querySelector(".mw-headline").textContent : null,
    jp:
      jp && jp.querySelector(".mw-headline") ? jp.querySelector(".mw-headline").textContent : null,
    en:
      en && en.querySelector(".mw-headline") ? en.querySelector(".mw-headline").textContent : null,
  };
}
