...
export let EQUIPMENTS_INTERNAL: { [s: string]: Equipment } = fs.existsSync(INTERNAL_EQUIPMENTS_PATH)
  ? JSON.parse(fs.readFileSync(INTERNAL_EQUIPMENTS_PATH).toString())
  : {};
...
export async function refreshEQImages() {
  ...
  for (let key of Object.keys(EQUIPMENTS_INTERNAL)) {
    ...
    if (eq.misc && eq.misc.animation)
      await fetchImage(
        eq.misc.animation,
        path.resolve(ROOT, "images/equipments.animation/" + cleanName + ".gif")
      );
  }
  console.log("\nDone");
}
...
export async function refreshEquipments() {
  ...
  for (let equipment_type of new JSDOM(data).window.document.querySelectorAll(
    ".mw-parser-output>div>ul>li>a"
  )) {
    // Equipments types layer
    let category = equipment_type.textContent;
    console.log("Refreshing Equipments type= " + category + "...");
    if (!fs.existsSync(path.resolve(ROOT, "web/equipments/" + category)))
      fs.mkdirSync(path.resolve(ROOT, "web/equipments/" + category));
    let doc = new JSDOM(
      await fetch(
        "https://azurlane.koumakan.jp" + equipment_type.getAttribute("href"),
        path.resolve(ROOT, "web/equipments/" + category + ".html")
      )
    ).window.document;
    ...
    for (let equipment_row of rows) {
      ...
      if (!equipment_row.firstElementChild || !equipment_row.firstElementChild.firstElementChild)
        continue; // some how jsdom's query selector is flaud, so dirty fix here, ignore it
      let href = `https://azurlane.koumakan.jp${equipment_row.firstElementChild.firstElementChild.getAttribute(
        "href"
      )}`;
      let name = equipment_row.firstElementChild.firstElementChild.getAttribute("title");
      process.stdout.write('Fetching "' + name + '" calling => ');
      EQUIPMENTS_INTERNAL[name] = parseEquipment(
        name,
        href,
        category,
        await fetch(
          ...
          path.resolve(
            ROOT,
            "web/equipments/" + category + "/" + name.replace(/\//g, "_") + ".html"
          )
        )
      );
...
function parseEquipment(name: string, href: string, category: string, body: string): Equipment {
  ...
  let eq: Equipment = {
    ...
    names: {
      ...
      cn: doc.querySelector('[lang="zh"]')
        ? doc.querySelector('[lang="zh"]').childNodes[1].textContent.trim()
        : null,
      jp: doc.querySelector('[lang="ja"]')
        ? doc.querySelector('[lang="ja"]').childNodes[1].textContent.trim()
        : null,
      kr: doc.querySelector('[lang="ko"]')
        ? doc.querySelector('[lang="ko"]').childNodes[1].textContent.trim()
        : null,
    },
  };
  let tiers: Tier[] = [];
...
function parseEquipmentStatsSlot(valueNode: Element): Stat {
  if (valueNode.childNodes.length === 6) {
    ...
    for (let i = 0; i < valueNode.children.length; i++)
      if (valueNode.children[i].textContent.trim())
        statValue[i] = parseEquipmentStatsSlot(valueNode.children[i]);
    return { type: "more_stats", stats: statValue };
  } else {
    let value = valueNode.textContent.trim();
    let rawData;
    if (value !== (rawData = value.replace(/(.+) → (.+) per (.+)/g, "$1|$2|$3"))) {
      //X → X' per P
      ...
      value !== (rawData = value.replace(/([^×]+) × ([^×]+) → ([^×]+) × ([^×]+)/g, "$1|$2|$3|$4"))
    ) {
      //X × C → X' × C
      rawData = rawData.split(/\|/);
      if (rawData[1].trim() === rawData[3].trim())
        return {
    ...
    } else if (value !== (rawData = value.replace(/([^×]+) × ([0-9]+)([^×→]+)/g, "$1|$2|$3"))) {
      // X × C U
      rawData = rawData.split(/\|/);
      return {
    ...
    } else if (value !== (rawData = value.replace(/([0-9.]+)([^0-9.]+)/g, "$1|$2"))) {
      // X U
      rawData = rawData.split(/\|/);
      return {
        type: "value_unit",
...
function parseEquipmentFit(eqfits: Element): Fits {
  ...
  for (let row of eqfits.getElementsByTagName("tr")) {
    ...
    if (row.children[1].textContent.trim() === "✘") fits[name] = null;
      ...
      fits[name] = row.children[1].getElementsByClassName("tooltiptext")[0]
        ? row.children[1].getElementsByClassName("tooltiptext")[0].textContent.trim()
        : "unspecified";
    if (fits[name] !== null && /^(.+) Only$/.test(fits[name]))
      fits[name] = fits[name].replace(/^(.+) Only$/, '"$1" only'); // XXX Only,
...
function parseEquipmentMisc(eqmisc: Element): any {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    else if (key === "patternAnimation")
      misc.animation = col.firstElementChild.firstElementChild.getAttribute("src");
    else if (key === "usedInGearLabFor")
      misc.usedFor = Array.from(col.children).map((elem) =>
        (<HTMLElement>(elem.querySelector("a") || elem)).title.replace(
          /\s+\(page does not exist\)/,
          ""
        )
      );
    else if (key === "createdInGearLabFrom")
      misc.madeFrom = Array.from(col.children).map((elem) =>
        (<HTMLElement>(elem.querySelector("a") || elem)).title.replace(
          /\s+\(page does not exist\)/,
          ""
        )
