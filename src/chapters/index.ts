...
const CHAPTERS_MIN_PATH = path.join(ROOT, 'dist', 'chapters.min.json');
const CHAPTERS_PATH = path.join(ROOT, 'dist', 'chapters.json');

export async function refreshChapter() {
    if (!fs.existsSync(path.join(ROOT, 'web', 'chapters'))) fs.mkdirSync(path.join(ROOT, 'web', 'chapters'));
    let names = parseChaptersNames(new JSDOM(await fetch("https://azurlane.koumakan.jp/Campaign", path.join(ROOT, 'web', 'chapters', 'index.html'))).window.document);
    let CHAPTERS = [];
    for (let i = 1; i <= 13; i++) {
        process.stdout.write("Refreshing Chapter " + i + " Details");
        CHAPTERS.push(parseChapter(new JSDOM(await fetch("https://azurlane.koumakan.jp/Chapter_" + i, path.join(ROOT, 'web', 'chapters', i + ".html"))).window.document, i, names));
