import {AirSupremacy, BaseExp, Chapter, EnemyLevel, MapPart, Reward, StarConditions} from "./chapter";
...
function parseMap(div: Element, code: string): MapPart {
    ...
    // @ts-ignore
    let map: MapPart = {
        title: div.querySelector(".mapbox-header").textContent,
        code: code,
    }
...
function parseEnermyLevel(div: Element): EnemyLevel {
    ...
    else if (div.childNodes.length > 7) info.boss = [...div.children].filter(n => (<HTMLElement>n).tagName === "A").map(e => (<HTMLElement>e).title);
...
function parseFleetRestriction(div: Element) {
    ...
    while (div.childNodes[i] && div.childNodes[i + 1]) {
        ...
        fleet_1_res[camelize(ne.title.replace(/\(.+\)/g, '').trim())] = parseInt(ce.textContent.replace(/[^\d]/g, ''));
        i += 2;
    }
    i += 3;
    let fleet_2_res: any = {};
    while (div.childNodes[i] && div.childNodes[i + 1]) {
        ...
        fleet_2_res[camelize(ne.title.replace(/\(.+\)/g, '').trim())] = parseInt(ce.textContent.replace(/[^\d]/g, ''));
        i += 2;
    }
    return {
        fleet1: fleet_1_res,
