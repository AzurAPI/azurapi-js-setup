...
export function deepToString(parent: Element) {
    ...
    if (parent.tagName === "IMG" && (<HTMLElement>parent).title) return `"${(<HTMLElement>parent).title}"`;
    if (parent.tagName === "IMG") return `"${(parent as HTMLImageElement).alt.replace(/(Icon)?.png/, '')}"`;
