...
export let SHIPS_INTERNAL: { [s: string]: Ship } = fs.existsSync(INTERNAL_SHIPS_PATH)
  ? JSON.parse(fs.readFileSync(INTERNAL_SHIPS_PATH).toString())
  : {};
export let SHIP_LIST = fs.existsSync(SHIP_LIST_PATH)
  ? JSON.parse(fs.readFileSync(SHIP_LIST_PATH).toString())
  : [];
...
export async function refreshShips() {
  ...
  if (!fs.existsSync(path.join(ROOT, "web", "ships")))
    fs.mkdirSync(path.join(ROOT, "web", "ships"));
  if (!fs.existsSync(path.join(ROOT, "web", "ships.gallery")))
    fs.mkdirSync(path.join(ROOT, "web", "ships.gallery"));
...
async function fetchShip(id: string, { name, url }: { name: string; url: string }): Promise<Ship> {
  ...
  let data = await fetch(
    `https://azurlane.koumakan.jp${url}`,
...
async function fetchShipList() {
  ...
  rows.forEach((table_ship) => {
    ...
    if ((<HTMLElement>columns[0]).tagName === "TD")
      LIST[id] = {
        id: id,
...
export async function refreshShipImages() {
  ...
  for (let key of keys) {
    ...
    for (let skin of ship.skins) {
      ...
      if (skin.image)
        await fetchImage(skin.image, root_folder + skin_folder + "image.png", thirdbar);
      if (skin.cn) await fetchImage(skin.cn, root_folder + skin_folder + "image.cn.png", thirdbar);
      if (skin.nobg)
        await fetchImage(skin.nobg, root_folder + skin_folder + "image.nobg.png", thirdbar);
      if (skin.chibi)
        await fetchImage(skin.chibi, root_folder + skin_folder + "chibi.png", thirdbar);
      else if (skin.name !== "Original Art" && !ship.unreleased)
        console.log(`${ship.names.en} is missing a chibi for ${skin.name}`);
      if (skin.background)
        await fetchImage(
          skin.background,
          path.join(
            ROOT,
            `images/backgrounds/${skin.background.substring(skin.background.lastIndexOf("/") + 1)}`
          ),
          thirdbar
        );
      //secondbar.increment();
    ...
    if (!fs.existsSync(path.resolve(ROOT, "images/skills", key)))
      fs.mkdirSync(path.join(ROOT, "images/skills", key));
    for (let skill of ship.skills)
      await getSkillIcon(
        key,
        skill,
        ship.skills,
        ship.skills.map((s) => transformSkillName(s.names.en)),
        thirdbar
      );
    shipCounter++;
  }
...
export function publishShips() {
  ...
  for (let key of keys) {
    ...
    for (let skin of ship.skins) {
      let skin_folder =
        skin.name
          ...
          .trim()
          .replace(/ +/g, "_") + "/";
      if (skin.image) skin.image = IMAGE_REPO_URL + root_folder + skin_folder + "image.png";
      else skin.image = undefined;
      if (skin.cn) skin.cn = IMAGE_REPO_URL + root_folder + skin_folder + "image.cn.png";
      else skin.cn = undefined;
      if (skin.nobg) {
        skin.bg = skin.image;
      ...
      skin.background = skin.background
        ? IMAGE_REPO_URL +
          "images/backgrounds/" +
          skin.background.substring(skin.background.lastIndexOf("/") + 1)
        : null;
    ...
    ship.skills = ship.skills.map((skill: Skill) =>
      publishSkill(
        ship.id,
        skill,
        ship.skills,
        ship.skills.map((skill: Skill) => transformSkillName(skill.names.en))
      )
    );
...
function publishSkill(id: string, skill: Skill, skills: Skill[], names: string[]) {
  ...
  if (names.filter((n) => n === name).length > 1)
    skill.icon = IMAGE_REPO_URL + "images/skills/" + id + "/" + skill.color + "." + name + ".png";
  else skill.icon = IMAGE_REPO_URL + "images/skills/" + id + "/" + name + ".png";
  return skill;
}

async function getSkillIcon(
  key: string,
  skill: Skill,
  skills: Skill[],
  names: string[],
  ...
  let spath;
  if (names.filter((n) => n === skillName).length > 1)
    spath = path.resolve(
      ROOT,
      "images/skills/" + key + "/" + skill.color + "." + skillName + ".png"
    );
