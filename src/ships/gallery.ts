...
export async function fetchGallery(name: string): Promise<{ skins: Skin[], gallery: GalleryItem[] }> {
    ...
    Array.from(doc.querySelectorAll(".mw-parser-output>.tabber>.tabbertab")).forEach(node => {
        ...
        if (tab.querySelector(".tabbertab")) image = {
            normal: tab.querySelector(".tabbertab[title=Default] .shipskin-image img") ? BASE + (<HTMLImageElement>tab.querySelector(".tabbertab[title=Default] .shipskin-image img")).src : null,
            nobg: tab.querySelector('.tabbertab[title="Without BG"] .shipskin-image img') ? BASE + (<HTMLImageElement>tab.querySelector('.tabbertab[title="Without BG"] .shipskin-image img')).src : null,
        ...
        skins.push({
            ...
            chibi: tab.querySelector(".shipskin-content .shipskin-chibi img") ? BASE + tab.querySelector(".shipskin-content .shipskin-chibi img").getAttribute("src") : null,
            info: info
        });
    });
    ...
    if (doc.getElementById("Artwork"))
        Array.from(doc.getElementById("Artwork").parentElement.nextElementSibling.children).filter(e => e.tagName === "DIV").forEach(box => gallery.push({
            ...
            url: galleryThumbnailUrlToActualUrl(BASE + box.querySelector("img").src)
        }));
