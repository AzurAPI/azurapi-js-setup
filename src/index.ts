  ...
  switch (args[0]) {
    ...
    case "ships":
      ...
      if (args.includes("publish")) {
        await publishShips();
        fs.copyFileSync(
          path.join(__dirname, "..", "dist", "ships.json"),
          path.join(__dirname, "..", "ships.json")
        );
        fs.copyFileSync(
    ...
    case "equipments":
      ...
      if (args.includes("publish")) {
        await publishEQ();
        fs.copyFileSync(
          path.join(__dirname, "..", "dist", "equipments.json"),
          path.join(__dirname, "..", "equipments.json")
        );
        fs.copyFileSync(
          path.join(__dirname, "..", "dist", "version.json"),
          path.join(__dirname, "..", "version-info.json")
