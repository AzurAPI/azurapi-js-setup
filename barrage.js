...
async function publish(barrages) {
    for (let barrage of barrages) {
        ...
        await fetchImage(barrage.icon, "images/barrages/" + barrage.id + "/icon.png");
        barrage.icon = REPO_URL + "images/barrages/" + barrage.id + "/icon.png";
        if (barrage.image) {
            await fetchImage(barrage.image, "images/barrages/" + barrage.id + "/image.gif");
            barrage.image = REPO_URL + "images/barrages/" + barrage.id + "/image.gif";
        }
    ...
    for (let r = 0, i = 0; r < list.length; r++) {
        ...
        if (tr.className === "expand-child") {
            ...
            barrages[i] = {
                ...
                type: type,
                icon: WIKI_URL + galleryThumbnailUrlToActualUrl(tr.firstElementChild.firstElementChild.src),
...
function fetchImage(url, localPath) {
    ...
    return new Promise((resolve, reject) => {
        if (fs.existsSync(localPath)) { // Check local file
            ...
            console.log("URL: " + url);
            request(url).pipe(fs.createWriteStream(localPath)).on('close', () => resolve()).on('error', e => reject(e));
        }
    });
}
