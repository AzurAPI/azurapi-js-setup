...
exports.refreshVoiceLines = async () => {
  for (let key in Object.keys(SHIPS)) {
    ...
    let text = await fetch(
      ship.wikiUrl + "/Quotes",
      "./web/ships.quotes/" + ship.names.en + ".html"
    );
...
function parseVoiceLines(doc, ship) {
  ...
  for (let tab of tabs) {
    ...
    while (i < tab.children.length) {
      if (
        tab.children[i].tagName === "H3" &&
        tab.children[i + 1] &&
        tab.children[i + 1].tagName === "TABLE"
      ) {
        skins.push({
  ...
  for (let server of Object.values(languages)) {
    for (let skin of server) {
      ...
      for (let line of skin.lines) {
        ...
        if (!skins[skinName][eventCamel])
          skins[skinName][eventCamel] = {
            event: line.event,
          };
        skins[skinName][eventCamel][line.language] = line.transcription;
        if (
          skins[skinName][eventCamel].audio &&
          line.audio &&
          skins[skinName][eventCamel].audio !== line.audio
        ) {
          skins[skinName][eventCamel].audioAlt = skins[skinName][eventCamel].audioAlt || [];
          skins[skinName][eventCamel].audioAlt.push(line.audio);
          if (fs.existsSync("./web/ships.quotes/" + ship.names.en + ".html"))
            fs.unlinkSync("./web/ships.quotes/" + ship.names.en + ".html");
...
function findElSkinName(skins, name) {
  ...
  for (let skin of skins) {
    ...
    if (
      skin.info.enClient &&
      (skin.info.enClient.trim().includes(name) || name.includes(skin.info.enClient.trim()))
    )
      return skin.name;
    if (
      skin.info.cnClient &&
      (skin.info.cnClient.trim().includes(name) || name.includes(skin.info.cnClient.trim()))
    )
      return skin.name;
    if (
      skin.info.jpClient &&
      (skin.info.jpClient.trim().includes(name) || name.includes(skin.info.jpClient.trim()))
    )
      return skin.name;
  }
  return name;
