...
async function refreshMemory(online) {
  ...
  if (!fs.existsSync("./web/memories/index.html") || online)
    fs.writeFileSync(
      "./web/memories/index.html",
      (data = await fetch("https://azurlane.koumakan.jp/Memories"))
    );
  else data = fs.readFileSync("./web/memories/index.html", "utf8");
  for (let galleryPack of new JSDOM(data).window.document.querySelectorAll(
    ".gallery.mw-gallery-packed"
  )) {
    for (let galleryBox of galleryPack.children) {
      let icon =
        "https://azurlane.koumakan.jp" +
        galleryThumbnailUrlToActualUrl(galleryBox.querySelector(".thumb img").src);
      ...
      if (!fs.existsSync("./web/memories/" + simple_name + ".html") || online)
        fs.writeFileSync("./web/memories/" + simple_name + ".html", (data = await fetch(url)));
...
async function publishMemoriesAndImages() {
  for (let key of Object.keys(MEMORIES)) {
    ...
    let thumbnailFile =
      "./images/memoryThumbnails/" +
      memory.thumbnail.substring(memory.thumbnail.lastIndexOf("/") + 1);
